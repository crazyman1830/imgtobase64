    ...
    try:
        ...
        try:
            ...
            queue_id = multi_file_handler.add_to_queue(
                files=temp_files, options=processing_options, priority=0
            )
        ...
        traceback.print_exc()
        return (
            jsonify({"error": f"배치 처리 시작 중 오류가 발생했습니다: {str(e)}"}),
            500,
        )
        ...
        if progress_info.status in ["completed", "cancelled", "error"]:
                ...
                response_data.update(
                    {
                        ...
                        "average_processing_time": processing_summary[
                            "average_processing_time"
                        ],
                        "total_processing_time": processing_summary[
            ...
            if failed_files:
                response_data["failed_file_details"] = [
                    {"file_path": f["file_path"], "error": f["error"]}
...
def batch_cancel(queue_id: str):
    ...
    try:
        ...
        return (
            jsonify({"error": f"배치 처리 취소 중 오류가 발생했습니다: {str(e)}"}),
            500,
        )
...
def batch_status():
    ...
    try:
        ...
        return (
            jsonify({"error": f"배치 상태 확인 중 오류가 발생했습니다: {str(e)}"}),
            500,
        )
...
def batch_cleanup():
    ...
    try:
        ...
        # 오래된 큐 정리
        max_age_hours = (
            request.json.get("max_age_hours", 24.0) if request.json else 24.0
        )
    ...
    return jsonify(
        {
            ...
            "timestamp": time.time(),
            "cache_enabled": (
                config.cache_enabled if hasattr(config, "cache_enabled") else True
            ),
...
def handle_join_queue(data):
    ...
    if queue_id:
        ...
        emit(
            "joined_queue",
            {"queue_id": queue_id, "message": f"큐 {queue_id}에 참가했습니다."},
        )
...
def handle_leave_queue(data):
    ...
    if queue_id:
        ...
        emit(
            "left_queue",
            {"queue_id": queue_id, "message": f"큐 {queue_id}에서 나갔습니다."},
        )
...
def handle_cancel_batch(data):
    ...
    if queue_id:
        ...
        if success:
            ...
            emit(
                "error",
                {"message": f"큐 {queue_id}를 찾을 수 없거나 취소할 수 없습니다."},
            )
    ...
    try:
        ...
        with Image.open(file_path) as image:
            ...
            if options.rotation_angle != 0:
                processed_image = image_processor.rotate_image(
                    processed_image, options.rotation_angle
                )

            # 뒤집기 적용
            if options.flip_horizontal:
                processed_image = image_processor.flip_image(
                    processed_image, "horizontal"
                )

            if options.flip_vertical:
                processed_image = image_processor.flip_image(
                    processed_image, "vertical"
                )
