...
class ErrorHandlingMiddleware:
    ...
    def _handle_http_exception(
        ...
        user_message = (
            self.error_templates.get(error.code) or 
            error.description or 
            f"HTTP {error.code} 오류가 발생했습니다."
        )
        ...
        if error.code == 413:  # Request Entity Too Large
            response_data['details'] = {
                'max_file_size_mb': getattr(self.container.get_config(), 'max_file_size_mb', 16),
    ...
    def _handle_application_error(
        ...
        formatted_response, status_code = self.error_formatter.format_error_response(error, context)
