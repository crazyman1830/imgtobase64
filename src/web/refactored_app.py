...
def _register_socketio_events(socketio: SocketIO, container: DIContainer) -> None:
    ...
    def handle_connect():
        ...
        socketio.emit(
            "connected",
            {"message": "WebSocket 연결이 성공했습니다.", "sid": request.sid},
        )
    ...
    def handle_ping(data):
        """핑 요청 처리"""
        socketio.emit("pong", {"timestamp": time.time(), "data": data})
...
if __name__ == "__main__":
    ...
    print(
        f"💾 Cache: {config.cache_backend if hasattr(config, 'cache_backend') else 'memory'}"
    )
    print("=" * 60)
