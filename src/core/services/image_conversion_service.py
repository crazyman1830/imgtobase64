...
class ImageConversionService:
    ...
    def _validate_input(self, file_path: str, options: Optional[ProcessingOptions]) -> None:
        """
        ...
        if not self._converter.validate_format(file_path):
            supported_formats = ', '.join(sorted(self._converter.get_supported_formats()))
    ...
    def _validate_processing_options(self, options: ProcessingOptions) -> None:
        ...
        if not isinstance(options, ProcessingOptions):
            raise ValidationError("Processing options must be a ProcessingOptions instance")
