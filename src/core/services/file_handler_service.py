...
class FileHandlerService(IFileHandler):
    ...
    def read_file_safe(self, file_path: str) -> Result[bytes, Exception]:
        ...
        try:
            ...
            if not os.path.isfile(file_path):
                return Result.failure(
                    FileNotFoundError(f"Path is not a file: {file_path}")
                )

            # Check if file is readable
            if not os.access(file_path, os.R_OK):
                return Result.failure(
                    PermissionError(f"Permission denied: Cannot read file {file_path}")
                )
            ...
            return Result.failure(
                FileSystemError(f"OS error reading file {file_path}: {str(e)}")
            )
        except Exception as e:
            return Result.failure(
                FileSystemError(f"Unexpected error reading file {file_path}: {str(e)}")
            )

    def save_file(
        self, content: str, output_path: str, overwrite: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if isinstance(e, (PermissionError, FileExistsError, FileSystemError)):
                ...
                raise FileSystemError(
                    f"Unexpected error saving file {output_path}: {str(e)}"
                )
    ...
    def save_file_safe(
        ...
        try:
            ...
            if not output_path or not isinstance(output_path, str):
                return Result.failure(
                    ValueError("Output path must be a non-empty string")
                )

            # Check if file already exists
            if os.path.exists(output_path) and not overwrite:
                return Result.failure(
                    FileExistsError(f"File already exists: {output_path}")
                )
            ...
            if output_dir and not os.path.exists(output_dir):
                try:
                    ...
                    return Result.failure(
                        PermissionError(
                            f"Cannot create directory {output_dir}: {str(e)}"
                        )
                    )
            ...
            return Result.failure(
                PermissionError(f"Error writing file {output_path}: {str(e)}")
            )
        except Exception as e:
            return Result.failure(
                FileSystemError(f"Unexpected error saving file {output_path}: {str(e)}")
            )
    ...
    def find_files(self, directory: str, pattern: str = "*") -> List[str]:
        ...
        try:
            ...
            if isinstance(e, (FileNotFoundError, PermissionError, FileSystemError)):
                ...
                raise FileSystemError(
                    f"Unexpected error finding files in {directory}: {str(e)}"
                )
    ...
    def find_files_safe(
        ...
        try:
            ...
            if not directory or not isinstance(directory, str):
                return Result.failure(
                    ValueError("Directory path must be a non-empty string")
                )
            ...
            if not os.path.isdir(directory):
                return Result.failure(
                    FileNotFoundError(f"Path is not a directory: {directory}")
                )
            ...
            return Result.failure(
                PermissionError(f"Error scanning directory {directory}: {str(e)}")
            )
        except Exception as e:
            return Result.failure(
                FileSystemError(
                    f"Unexpected error scanning directory {directory}: {str(e)}"
                )
            )
    ...
    def find_image_files(self, directory_path: str) -> List[str]:
        ...
        try:
            ...
            if isinstance(e, (FileNotFoundError, PermissionError, FileSystemError)):
                ...
                raise FileSystemError(
                    f"Unexpected error finding image files in {directory_path}: {str(e)}"
                )

    def find_image_files_safe(
        ...
        try:
            ...
            if not os.path.exists(directory_path):
                return Result.failure(
                    FileNotFoundError(f"Directory not found: {directory_path}")
                )

            # Check if it's a directory
            if not os.path.isdir(directory_path):
                return Result.failure(
                    FileNotFoundError(f"Path is not a directory: {directory_path}")
                )
            ...
            return Result.failure(
                PermissionError(f"Error scanning directory {directory_path}: {str(e)}")
            )
        except Exception as e:
            return Result.failure(
                FileSystemError(
                    f"Unexpected error scanning directory {directory_path}: {str(e)}"
                )
            )
    ...
    def file_exists_safe(self, file_path: str) -> Result[bool, Exception]:
        ...
        try:
            ...
            return Result.failure(
                FileSystemError(f"Error checking file existence {file_path}: {str(e)}")
            )
    ...
    def get_file_size(self, file_path: str) -> int:
        ...
        try:
            ...
            if isinstance(e, (FileNotFoundError, FileSystemError)):
                ...
                raise FileSystemError(
                    f"Unexpected error getting file size for {file_path}: {str(e)}"
                )
    ...
    def get_file_size_safe(self, file_path: str) -> Result[int, Exception]:
        ...
        try:
            ...
            return Result.failure(
                FileSystemError(f"OS error getting file size for {file_path}: {str(e)}")
            )
        except Exception as e:
            return Result.failure(
                FileSystemError(
