...
class ParallelProcessStrategy(ProcessingStrategy):
    ...
    def __init__(
        self, parallel_processor: ParallelProcessor, use_cpu_intensive: bool = False
    ):
        ...
        try:
            ...
            def run_parallel():
                if self.use_cpu_intensive:
                    return self.parallel_processor.process_cpu_intensive_batch(
                        tasks, processor_func
                    )
                else:
                    return self.parallel_processor.process_io_intensive_batch(
                        tasks, processor_func
                    )

            results: List[ConversionResult] = await loop.run_in_executor(
                None, run_parallel
            )
