...
from .log_formatters import SecurityFormatter, PerformanceFormatter, JSONFormatter
...
class SecurityLogHandler(RotatingFileHandler):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        security_indicators = [
            'ip_address', 'user_agent', 'security_event', 'threat_level',
            'scan_result', 'rate_limit', 'suspicious_activity'
        ]
...
class PerformanceLogHandler(RotatingFileHandler):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        performance_indicators = [
            'processing_time', 'memory_usage', 'metric_name', 'metric_value',
            'operation_time', 'performance_metric'
        ]
...
class MetricsHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        ...
        with self.lock:
            ...
            # Update by logger
            logger = record.name
            self.metrics['logs_by_logger'][logger] = self.metrics['logs_by_logger'].get(logger, 0) + 1
