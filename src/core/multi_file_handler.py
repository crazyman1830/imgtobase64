...
from ..models.models import ConversionResult, ProcessingQueueFullError, ConversionError
...
from .parallel_processor import (
    get_parallel_processor, ProcessingTask, ParallelProcessor
)
...
class MultiFileHandler:
    ...
    def __init__(self, max_concurrent: int = 3, max_queue_size: int = 100, enable_memory_optimization: bool = True):
        """
        ...
        self.parallel_processor = get_parallel_processor(
            ...
            max_memory_mb=500
        ) if enable_memory_optimization else None
    ...
    def get_queue_info(self, queue_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            processing_count = sum(1 for item in queue.items 
                                 if item.started_time is not None and item.completed_time is None)
            completed_count = sum(1 for item in queue.items if item.completed_time is not None)
    ...
    def _calculate_estimated_time(self, queue: ProcessingQueue) -> float:
        ...
        completed_items = [item for item in queue.items if item.completed_time is not None]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            active_queues = sum(1 for q in self._queues.values() if q.status == "processing")
            completed_queues = sum(1 for q in self._queues.values() if q.status == "completed")
            cancelled_queues = sum(1 for q in self._queues.values() if q.status == "cancelled")
    ...
    def get_processing_summary(self, queue_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            total_files = len(queue.items)
            completed_files = sum(1 for item in queue.items if item.completed_time is not None)
            successful_files = sum(1 for item in queue.items 
            ...
            total_processing_time = queue.completed_time - queue.started_time if queue.started_time and queue.completed_time else 0
            ...
            return {
                ...
                'success_rate': (successful_files / completed_files * 100) if completed_files > 0 else 0,
    ...
    def _handle_memory_warning(self, threshold_type: str, usage: Dict[str, Any]) -> None:
        """
        ...
        if self.gc_optimizer:
            ...
            print(f"Memory warning: {usage['rss_mb']:.1f}MB used. "
                  f"GC collected {gc_result['objects_collected']} objects.")
    
    def _handle_memory_critical(self, threshold_type: str, usage: Dict[str, Any]) -> None:
        """
        ...
        if self.gc_optimizer:
            ...
            print(f"Critical memory usage: {usage['rss_mb']:.1f}MB used. "
    ...
    async def _process_single_file_optimized(
        ...
        async with semaphore:
            ...
            try:
                ...
                with memory_context['memory_monitor'].monitor_operation(f"process_file_{Path(item.file_path).name}") as monitoring:
                    # Run the processor function in thread pool to avoid blocking
                    loop = asyncio.get_event_loop()
                    result = await loop.run_in_executor(
                        self._thread_pool,
    ...
    def _process_with_memory_optimization(
        ...
        try:
            ...
            raise ConversionError(f"Failed to process file with memory optimization: {str(e)}")
    ...
    def optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        if cleaned_queues > 0:
            optimization_results['actions_taken'].append(f'Cleaned {cleaned_queues} old queues')
        ...
        if self.gc_optimizer:
            ...
            optimization_results['actions_taken'].append(f'Collected {gc_result["objects_collected"]} objects')
        ...
        if self.memory_monitor:
            ...
            optimization_results['memory_freed_mb'] = (
                optimization_results['initial_memory_mb'] - optimization_results['final_memory_mb']
            )
    ...
    def process_queue_parallel(
        ...
        try:
            ...
            if use_cpu_intensive:
                results = self.parallel_processor.process_cpu_intensive_batch(tasks, processor_func)
            else:
                results = self.parallel_processor.process_io_intensive_batch(tasks, processor_func)
