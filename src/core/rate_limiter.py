...
class RateLimiter:
    ...
    def check_rate_limit(self, ip_address: str, raise_on_limit: bool = True) -> RateLimitStatus:
        """
        ...
        with self._lock:
            ...
            is_limited = (
                ...
                day_count >= self.config.requests_per_day or
                burst_count >= self.config.burst_limit
            )
    ...
    def _cleanup_ip_requests(self, ip_address: str, current_time: float) -> None:
        ...
        while (self._minute_requests[ip_address] and 
               self._minute_requests[ip_address][0] < minute_cutoff):
            self._minute_requests[ip_address].popleft()
        
        # Clean up hour requests (older than 3600 seconds)
        hour_cutoff = current_time - 3600
        while (self._hour_requests[ip_address] and 
               self._hour_requests[ip_address][0] < hour_cutoff):
            self._hour_requests[ip_address].popleft()
        
        # Clean up day requests (older than 86400 seconds)
        day_cutoff = current_time - 86400
        while (self._day_requests[ip_address] and 
               self._day_requests[ip_address][0] < day_cutoff):
        ...
        while (self._burst_requests[ip_address] and 
               self._burst_requests[ip_address][0] < burst_cutoff):
    ...
    def _cleanup_old_entries(self, current_time: float) -> None:
        ...
        ips_to_remove = [
            ip for ip, last_seen in self._last_seen.items()
        ...
        if ips_to_remove:
            self.logger.info(f"Cleaned up rate limit data for {len(ips_to_remove)} old IPs")
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            total_minute_requests = sum(len(reqs) for reqs in self._minute_requests.values())
            total_hour_requests = sum(len(reqs) for reqs in self._hour_requests.values())
