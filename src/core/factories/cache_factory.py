...
class CacheFactory:
    ...
    DEFAULT_MEMORY_CONFIG = {"max_entries": 1000, "max_age_seconds": 3600}  # 1 hour
    ...
    def create_memory_cache(
        self, max_entries: int = 1000, max_age_seconds: int = 3600, **kwargs
    ) -> ICacheManager:
        """
        ...
        try:
            ...
            return CacheManagerService(backend_type="memory", backend_config=config)
    ...
    def create_disk_cache(
        ...
        **kwargs,
        ...
        try:
            ...
            return CacheManagerService(backend_type="disk", backend_config=config)
    ...
    def create_redis_cache(
        ...
        **kwargs,
        ...
        try:
            ...
            return CacheManagerService(backend_type="redis", backend_config=config)
    ...
    def create_hybrid_cache(
        self, primary_config: Dict[str, Any], fallback_config: Dict[str, Any]
    ) -> ICacheManager:
        """
    ...
    def create_auto_cache(
        ...
        if redis_host:
            try:
                return self.create_redis_cache(host=redis_host, port=redis_port)
            except CacheError:
        ...
        try:
            return self.create_disk_cache(
                cache_dir=cache_dir, max_size_mb=max_disk_size_mb
            )
        except CacheError:
            pass  # Fall back to memory cache

        # Fall back to memory cache
        return self.create_memory_cache(max_entries=max_memory_entries)
