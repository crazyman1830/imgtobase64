...
from ..domain.exceptions.base import ErrorCode, ImageConverterError
from ..domain.exceptions.cache import CacheError, CacheReadError, CacheWriteError
from ..domain.exceptions.file_system import (
    FileNotFoundError,
    FileSystemError,
    PermissionError,
)
from ..domain.exceptions.processing import (
    ConversionError,
    CorruptedFileError,
    ProcessingError,
)
from ..domain.exceptions.queue import ProcessingQueueFullError, QueueError
...
class ErrorHandler:
    ...
    def handle_error(
        ...
        if error_type in self.recovery_strategies:
            try:
                ...
                self.logger.warning(
                    f"Recovery strategy failed for {error_id}: {recovery_error}"
                )
    ...
    def _log_error(self, error_context: ErrorContext):
        ...
        if error_context.severity == ErrorSeverity.CRITICAL:
            self.logger.critical(
                f"Critical error {error_context.error_id}: {json.dumps(log_data)}"
            )
        elif error_context.severity == ErrorSeverity.HIGH:
            self.logger.error(
                f"High severity error {error_context.error_id}: {json.dumps(log_data)}"
            )
        elif error_context.severity == ErrorSeverity.MEDIUM:
            self.logger.warning(
                f"Medium severity error {error_context.error_id}: {json.dumps(log_data)}"
            )
        else:
            self.logger.info(
                f"Low severity error {error_context.error_id}: {json.dumps(log_data)}"
            )
    ...
    def _recover_from_cache_error(self, error_context: ErrorContext) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Attempting cache error recovery for {error_context.error_id}"
        )
    ...
    def _recover_from_queue_error(self, error_context: ErrorContext) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Attempting queue error recovery for {error_context.error_id}"
        )
    ...
    def wrap_result_error(
        self, result: Result, operation: Optional[str] = None
    ) -> Result:
        """
        ...
        generic_error = ImageConverterError(
            message=str(result.error), error_code=ErrorCode.UNKNOWN_ERROR
        )
