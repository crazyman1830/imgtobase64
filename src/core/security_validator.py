...
class SecurityValidator:
    ...
    def __init__(
        ...
        self.allowed_mime_types = allowed_mime_types or self.DEFAULT_ALLOWED_MIME_TYPES.copy()
    ...
    def validate_file_header(self, file_path: str) -> tuple[bool, str]:
        ...
        try:
            ...
            for signature, format_type in self.IMAGE_SIGNATURES.items():
                ...
                if signature == b'RIFF' and header.startswith(b'RIFF') and b'WEBP' in header[:12]:
        ...
        try:
            ...
            try:
                ...
                if not file_size_valid:
                    ...
                    scan_result.add_warning(f"File size exceeds limit of {self.max_file_size} bytes")
            ...
            try:
                ...
                if not mime_valid:
                    ...
                    scan_result.add_warning(f"Invalid or disallowed MIME type: {detected_mime}")
            except Exception as e:
            ...
            try:
                ...
                if not header_valid:
                    scan_result.is_safe = False
                    scan_result.threat_level = "high"
                    scan_result.add_warning(f"Invalid file header or unknown format: {detected_format}")
            except Exception as e:
            ...
            try:
                advanced_safe, advanced_warnings = self._perform_advanced_security_checks(file_path)
    ...
    def _scan_file_content(self, file_path: str, chunk_size: int = 8192) -> tuple[bool, list[str]]:
        """
                ...
                while bytes_read < max_scan_size:
                    ...
                    for pattern in self.SUSPICIOUS_PATTERNS:
                        if pattern in chunk_lower:
                            detected_threats.append(f"Suspicious pattern detected: {pattern.decode('utf-8', errors='ignore')}")
                    
                    # Check for advanced threat patterns
                    for pattern in self.ADVANCED_THREAT_PATTERNS:
                        if pattern in chunk_lower:
                            detected_threats.append(f"Advanced threat pattern detected: {pattern.decode('utf-8', errors='ignore')}")
    ...
    def _perform_advanced_security_checks(self, file_path: str) -> tuple[bool, list[str]]:
        """
            ...
            if file_ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp']:
                ...
                try:
                    ...
                    if not has_image_signature:
                        warnings.append(f"File extension {file_ext} doesn't match content")
                        is_safe = False
